      ...
      shinythemes::themeSelector(),
          ...
          tabPanel("Apercu général sur les SVM",
                    # mainPanel(column(12, #offset = 2, includeMarkdown("./resume.rmd"))   
                                        ...
                                        mainPanel( 
                                          tabsetPanel( 
                                            tabPanel("C'est quoi les svm?" ,align="center"), 
                                            tabPanel("Hypothèse1"),  
                                            tabPanel("Hypothèse2"), 
                                            tabPanel("Hypothèse3") 
          ...
          tabPanel(
               title= "Exploration",  
                     sidebarLayout(  
                      ...
                      mainPanel( 
                            tabsetPanel( 
                            tabPanel(p(icon("bar-chart-o"),"Distributions") ,plotOutput("G_descript")), 
                            tabPanel(p(icon("table"),"La Base"),DT::dataTableOutput("donnee")),  
                            tabPanel(p(icon("table"),"Corrélations "),tableOutput("corr")), 
                            tabPanel(p(icon("area-chart"),"Scatterplot"),plotOutput("scatter")), 
                            tabPanel(p(icon("area-chart"),"Boxplot"),plotlyOutput("cible"))
                           ) 
                    )# mainpanel
                  ) #sidebarlayout
           ),#tabpanel 
